...
/*void read_day_data(FILE *file, LIST *list, int month, int day, int year, int len){
	char buffer[STR_LEN];
	...
	while(!feof(file)){
	  ...
	  if(len > 24){
	  	read_by_month(file, list, month, year);
	  }
	  if(len == 24){
	  	read_by_day(file, list, month, day, year);
	  }
	  IF(len == 12){
	  	read_by_year(file, list, year);
	  }
	}
}*/
...
int main(void){
  ...
  //count_node(data, counts, analysis);
  //list_free(data);
